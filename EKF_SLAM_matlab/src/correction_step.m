...
for i = 1:m
	...
	if ~observedLandmarks(landmarkId)
        ...
        bearing = z(i).bearing;
		mu(2+2*landmarkId) = mu(1) + z(i).range * cos(bearing + mu(3));
        mu(3+2*landmarkId) = mu(2) + z(i).range * sin(bearing + mu(3));
	...
	% Z_hat = h(x)
    delta = [mu(2+2*landmarkId)-mu(1); mu(3+2*landmarkId)-mu(2)];
    q = norm(delta);
    qq = q*q;
    Z_hat(2*i-1) = q;
    ...
    Hi = zeros(2, 2*N+3);
    Hi(:, 1:3) = H3;
    Hi(:, 2+2*landmarkId:3+2*landmarkId) = H2;
